...
export class VernierVisRow {
    ...
    async startRecording(dirHandle: FileSystemDirectoryHandle) {
        if (this.forceIsOn()) {
            const forceFileHandle = await dirHandle.getFileHandle(
                `Vernier_${this.deviceName}_Force.arrow`,
                { create: true },
            );
            this.forceStreamer = new ArrowStreamer(
                forceFileHandle,
                this.getForceSchema(),
            );
        }
        ...
        if (this.brIsOn()) {
            const brFileHandle = await dirHandle.getFileHandle(
                `Vernier_${this.deviceName}_BR.arrow`,
                { create: true },
            );
            this.brStreamer = new ArrowStreamer(
                brFileHandle,
                this.getBRSchema(),
            );
        }
    ...
    static hasAnyActiveStream() {
        ...
        for (let i = 0; i < VernierVisRow.vernierVisRows.length - 1; i++) {
            ...
            if (row.forceDiv !== undefined || row.brDiv !== undefined)
                return true;
        }
        return false;
    }
    ...
    async init() {
        this.sensorRowDiv = createDiv(
            `vernierSensorDiv-${VernierVisRow.vernierRowID}`,
            this.parent,
            ["polar-sensor-row", "flexbox"],
        );

        this.optionDiv = createDiv(`vernierOptionDiv`, this.sensorRowDiv, [
        ...
        const deviceInfoDiv = createDiv("deviceInfoDiv", this.optionDiv, [
            "device-info",
        ]);

        const disconnectDiv = createDiv("disconnectDiv", undefined, [
            "flexbox",
            "disconnect",
        ]);
        ...
        if (VernierVisRow.vernierVisRows.length > 0) {
            VernierVisRow.vernierVisRows[
                VernierVisRow.vernierVisRows.length - 1
        ...
        const forceCtrlDiv = createDiv("forceCtrlDiv", this.dataCtrl, [
            "half-width",
        ]);
        const forceSwitch = createSwitch(
            "Force",
            this.onToggleForce,
            () => VernierVisRow.vernierRowID,
        );
        forceCtrlDiv.appendChild(forceSwitch);
        this.forceSwitchInput = forceSwitch.querySelector(
            "input",
        ) as HTMLInputElement;

        const brCtrlDiv = createDiv("brCtrlDiv", this.dataCtrl, ["half-width"]);
        const brSwitch = createSwitch(
            "Resp",
            this.onToggleBR,
            () => VernierVisRow.vernierRowID,
        );
        brCtrlDiv.appendChild(brSwitch);
        this.brSwitchInput = brSwitch.querySelector(
            "input",
        ) as HTMLInputElement;
        ...
        this.brInfo = new SmoothieTSInfo(
            undefined,
            undefined,
            "BR: -- bpm",
            "left",
        ...
        if (duplicateInd < 0) {
            ...
            this.forceSensor =
                VernierVisRow.vernierVisRows[duplicateInd].forceSensor;
        }
    ...
    onToggleForce = (ev: any) => {
        if (ev.target?.checked) {
            let width_class =
                this.brDiv === undefined ? "full-width" : "half-width";
            if (this.brDiv) {
                this.brDiv.classList.remove("full-width");
                this.brDiv.classList.add("half-width");
            }
            this.forceDiv = createDiv("forceDiv", this.visContainerDiv, [
                "float-left",
                "almost-full-height",
                width_class,
            ]);
            ...
            this.forceResizeObserver = new ResizeObserver(() =>
                this.forceResize!(),
            );
    ...
    onToggleBR = (ev: any) => {
        if (ev.target?.checked) {
            let width_class =
                this.forceDiv === undefined ? "full-width" : "half-width";
            if (this.forceDiv) {
                this.forceDiv.classList.remove("full-width");
                this.forceDiv.classList.add("half-width");
            }
            this.brDiv = createDiv("brDiv", this.visContainerDiv, [
                "float-left",
                "almost-full-height",
                width_class,
            ]);
    ...
    calculateBreathingRate(force: number, timestamp: number) {
        ...
        while (
            this.forceTimeWindow.length > 0 &&
            timestamp - this.forceTimeWindow[0] > 20000
        ) {
        ...
        for (let i = 0; i < this.forceDataWindow.length; i++) {
            ...
            for (
                ...
                j <= Math.min(this.forceDataWindow.length - 1, i + 2);
                j++
            ) {
    ...
    async disconnect() {
        ...
        if (
            this.sensorRowDiv &&
            this.parent &&
            this.parent.contains(this.sensorRowDiv)
        ) {
        ...
        try {
            if (this.forceSensor && this.handleDataBound) {
                if (typeof this.forceSensor.removeListener === "function") {
                    this.forceSensor.removeListener(
                        "value-changed",
                    ...
                    typeof this.forceSensor.removeEventListener === "function"
                ) {
                    this.forceSensor.removeEventListener(
                        "value-changed",
                        this.handleDataBound,
                    );
                }
            ...
            console.error("Error removing Vernier listener", e);
        }
        ...
        if (duplicateInd < 0) {
            if (this.device) {
                try {
                    ...
                    console.error("Error stopping Vernier device", e);
                }
            }
        }
